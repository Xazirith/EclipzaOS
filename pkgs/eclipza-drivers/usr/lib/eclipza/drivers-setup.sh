...
# Make sure base tools are present (safe to re-run)
...
# Helper: add line once
add_line_once(){ local line="$1" file="$2"; grep -qxF "$line" "$file" 2>/dev/null || echo "$line" | sudo tee -a "$file" >/dev/null; }
...
if (( has_nvidia )); then
  ...
  kern="$(uname -r)"
  if [[ "$kern" =~ ^6\..* ]]; then
    # Stock Arch kernels usually fine with binary nvidia
...
# Sanity log
log "Done. If NVIDIA was installed/changed, reboot is recommended."
exit 0
